...
/************************************************************************************************
* Function Name: OSMemGetOpt
* Decription   : 从指定的内存中获取一段存储区
* Input        : MemPtr-内存大块指针, Retry-如果失败重试的次数, Interval-每次重试之间等待的时间(ms)
* Output       : 返回获取内存的指针
* Others       : 此函数非uC/OS-II源码,为项目方便自己添加
************************************************************************************************/
void *OSMemGetOpt(OS_MEM *MemPtr, INT8U Retry, INT32U Interval)
{
    INT8U err, *msg;

    while (Retry--) {
        msg = OSMemGet(MemPtr, &err);
        if ((void *)0 != msg || 0 == Retry) {
            break;
        }
        OSTimeDly(Interval);
    }
