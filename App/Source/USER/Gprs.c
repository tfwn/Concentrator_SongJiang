...
bool Gprs_TxPkgHandle_New(uint8 Cmd, uint8 *BufPtr)
{
    ...
    // 增加头部信息
    // 4 个字节的登录帧标识。如，上海荣旗 = SHRQ = 53 48 52 51
    ptr->Buffer[0] = 0x53;
    ptr->Buffer[1] = 0x48;
    ptr->Buffer[2] = 0x52;
    ptr->Buffer[3] = 0x51;

    // 8 个字节的设备序列号( MAC 地址) = 设备生产日期 + 序列号
    memcpy(&ptr->Buffer[4], Concentrator.GprsParam.MacAddr, 8);

	// 16 个字节的 SIM 卡号，不足16字节的后边补空格
...
* Decription   : Gprs 登录帧
* Input        : BufPtr-指向数据的指针
* Output       : TRUE-正确,FALSE-错误
* Others       : 登录帧标识[4] + 设备序列号[8] + SIM卡号[16](11+5空格) + 回车[1] + 换行[1]
************************************************************************************************/
uint16 Gprs_SignInPkgHandle(uint8 *BufPtr)
{
	uint16 len = 0;
    // 上海荣旗 = SHRQ = 53 48 52 51
    ...
    while (TRUE) {
        switch (Gprs.Cmd) {
			...
			case GprsSignIn:
				...
				len = Gprs_SignInPkgHandle(cmd); // 登录帧
				if (Quectel_M35 == Gprs.ModuleType) {
				...
				bufPtr->Length = Gprs_SignInPkgHandle(bufPtr->Buffer); // 登录帧
				Gprs_OutputDebugMsg(TRUE, ">>>登录包");
            ...
            case GprsUpdataRtc:
                ...
                // 中国默认国际时间服务器地址为"210.72.145.44" ，端口号是 123
                // 上海交通大学校时服务器 "202.120.2.101",123
                // windows上查看到的校时服务器 "time.nist.gov",13
                // AT+QNTP 指令间隔最好在64s以上，否则容易失败！
                if (Quectel_M35 == Gprs.ModuleType) {
                ...
                if ((void *)0 == ret) {
                    ...
                    eventFlag &= ~FLAG_GPRS_UPDATA_RTC;
                    AtQntpErr++;
                    RTCTimingTimer = 100; 			// 如果校时失败则100s后重试
                    Gprs.Cmd = GprsConnectIdleStatus;
                    break;
                }
                ...
                if ((void *)0 != msg) {
                    ...
                    for( i = 0; i < bufPtr->Length; i++ ){
                        if( (bufPtr->Buffer[i] == 0x22) && (bufPtr->Buffer[i+3] == 0x2f) && (bufPtr->Buffer[i+6] == 0x2f)){
                            ...
                            len+=3;
                            if(0x2B == bufPtr->Buffer[len-1]){//正时区，需要用世界时间 + 时区/4
                                addsub = 1;
                                if (Quectel_M35 == Gprs.ModuleType) {
                                    timezone = 32;// M35返回信息中时区信息为0，手动增加中国时区，需要用世界时间 + 时区/4
                                }else{
                                    timezone = (BcdToBin(bufPtr->Buffer[len])-30)*10 + (BcdToBin(bufPtr->Buffer[len+1])-30);
                                }
                            }else if(0x2D == bufPtr->Buffer[len-1]){//负时区
                                addsub = 0;
                                if (Quectel_M35 == Gprs.ModuleType) {
                                    timezone = 32;// M35返回信息中时区信息为0，手动增加中国时区，需要用世界时间 + 时区/4
                                }else{
            ...
            case GprsCheckConnect:
                if( GPRS_SOFT_UP_EN == GprsSoftUp ){
                    ...
                    bufPtr->Length = 0;
                    bufPtr->Property.PortNo = Uart_Gprs;
                    bufPtr->Buffer[bufPtr->Length++] = 0x24;// 心跳为一个英文的美元字符 "$" = 0x24
                    ret = Gprs_ConnectProc((char *)(bufPtr->Buffer), bufPtr->Length, "SEND OK", (void *)0, TIME_DELAY_MS(5000), TIME_DELAY_MS(500), 1, FALSE);
                    if ((void *)0 == ret) {
                        OSMemPut(SmallMemoryPtr, cmd);
                        Gprs.Cmd = GprsCloseTcp;
                    ...
                    for (loop = 0; loop < 5; loop++) {
                        ...
                        bufPtr = (PORT_BUF_FORMAT *)msg;
                        if (0x25 == bufPtr->Buffer[bufPtr->Length-3] || 0x25 == bufPtr->Buffer[0]) {// 心跳成功返回一个英文百分号字符 "%" = 0x25
                            Gprs.HeartbeatRetryTime = 0;
                            Gprs.Cmd = GprsConnectIdleStatus;
                            Gprs.HeartbeatInterval = Concentrator.GprsParam.HeatBeatPeriod * 10;
                            if (FALSE == Gprs.Online) {
